...
void Animation::loadTracks(
    const std::vector<AnimationReader::AnimationTrack>& tracks) {
    if (!this->_tracks.empty()) {
        error("Tracks have already been loaded for this animation.");
        // throw std::runtime_error("Tracks have already been loaded for this
        // animation.");
    }
    if (tracks.empty()) {
    ...
    for (uint i = 0; i < tracks.size(); i++) {
        ...
        if (this->_frameCount != tracks[i].frameCount) {
            ...
            // throw std::runtime_error("All tracks must have the same number of
            // frames.");
        }
    }
